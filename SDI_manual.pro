...
binstringa = strtrim(string(7.8125*d))
...
risp = 'no'
print
print, 'digitare un carattere qualunque per terminare'
print, 'digitare si per salvare le curve accorpate '
while ((risp EQ 'si')OR(risp EQ 'no')) do begin
	read, risp
	if ((risp EQ 'si')AND(d NE 1)) then begin
	  print
	  print, 'hai risposto ', risp
	  print, 'salvataggio curve accorpate'
	  binmax = FIX(13568/d)
	  close, 10, 11, 12, 13

	  openw, 10, './OutputFile/LS1rebin.asc'
		Xrebin = [1.0,0,0,0]##xr
		Yrebin = [1.0,0,0,0]##yr
		i = 0ul
		while (i LT binmax) do begin
		   printf, 10, Xrebin[i], Yrebin[i]
		   i = i + 1
		endwhile			
			  	

	  close, 10
	  openw, 11, './OutputFile/LS2rebin.asc'
		Xrebin = [0,1.0,0,0]##xr
		Yrebin = [0,1.0,0,0]##yr
		i = 0ul
		while (i LT binmax) do begin
		   printf, 11, Xrebin[i], Yrebin[i]
		   i = i + 1
		endwhile
	  	

	  close, 11
	  openw, 12, './OutputFile/LS3rebin.asc'
		Xrebin = [0,0,1.0,0]##xr
		Yrebin = [0,0,1.0,0]##yr
		i = 0ul
		while (i LT binmax) do begin
		   printf, 12, Xrebin[i], Yrebin[i]
		   i = i + 1
		endwhile
	  

	  close, 12
	  openw, 13, './OutputFile/LS4rebin.asc'
		Xrebin = [0,0,0,1.0]##xr
		Yrebin = [0,0,0,1.0]##yr
		i = 0ul
		while (i LT binmax) do begin
		   printf, 13, Xrebin[i], Yrebin[i]
		   i = i + 1
		endwhile
