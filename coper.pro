...
;print,''
	...
	while (cur LE (lun-1)) AND (NOT EOF(1)) do begin  ;W1 *** ciclo su gli archivi da confrontare
		...
		print, 'op in archivio locale ', op_loc ;fix(op_loc)
		print, 'op in archivio remoto ', b ;fix(b)
	...
	printf, 2, ' tempo totale copertura osservazione grbm ore/min/sec', converti(totale)	
	 ...
	 total = 6400000
	 ;total = archivio[lun-2].tempass
	 Y1 = intarr(total)

al = 0
puntasec = 0
while (al LE (lun - 2)) do begin

	if (archivio[al].presente EQ si)then Y1[puntasec] = 1
	if ((archivio[al].presente EQ si)AND(archivio[al].grbm_on EQ si)) then Y1[puntasec] = 2
	if (archivio[al].presente NE si)then Y1[puntasec] = 0
	print, 'presente ', archivio[al].presente
	print, 'grbm_on ', archivio[al].grbm_on
	print, 'condizione ', al+1, Y1[puntasec]

if al EQ 0 then begin 
	puntasec = puntasec + archivio[0].tempass
endif else begin
	puntasec = archivio[al].tempass + 1
...
endwhile 
	 ...
	 X = (archivio.tempass)/86400
	 Y = (archivio.copertura)/86400 
