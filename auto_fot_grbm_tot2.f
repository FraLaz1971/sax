...
c       CREA CURVE TOTALI NELLA DIRECTORY CORRENTE
c       LEGGE I NUMERI DI OSSERVAZIONE DAL FILE 'NOBS.TXT'
...
c       ESTRAI IL NUMERO DELL'OP
	
	tempfile='AcOP.txt'
	open(unit=207, access='sequential', file=tempfile, status='old' )
	read(207,139)opid
	path='op_'//opid 
	write(*,*)'extracting data from OP '//opid
c	write(*,*)path
	

c       ESTRAI IL NUMERO DELL'OSSERVAZIONE
	
	tempfile='nobs.txt'
	open(unit=207, access='sequential', file=tempfile, status='old' )
	read(207,24)on
	write(*,*)'creating total curves for observation '//on
c	write(*,*)on
...
2002		   close(10)
     ...
     1          b//'_'//a2//'.tot.out'
		...
		do iii=1,itot
		 ...
		 if (tot_data(iii).ge.999.99) then
		   write(*,*)'tot_data = '
		   write(*,*)tot_data(iii)
		   write(*,*)'errore = '
		   write(*,*)sqrt(tot_data(iii))
c		   tot_data(iii) = tot_data(iii) 
		 end if
                 write(99,400)time_bin,tot_data(iii),sqrt(tot_data(iii))
c		 write(*,*)'tot_data = '
c		 write(*,*)tot_data(iii)
c		 write(*,*)'errore = '
c		 write(*,*)sqrt(tot_data(iii))		
		end do

               close(99)
400    	       format(1x,f11.5,3x,f8.2,3x,f10.5)
	end do
5001	continue
