...
openw, 25, 'SDIresult.txt'; APERTO FILE DI OUTPUT
...
printf, 25, separatore
printf, 25
printf, 25, 'OP  ', tempopid
printf, 25
printf, 25, 'osservazione ', tempobs
printf, 25
printf, 25, separatore

print,'Nell'' OP ed osservazione specificati sono presenti i seguenti trigger:'
printf, 25,'curve totali ad alta risoluzione presenti :'
printf, 25
...
printf,25 ,'files presenti : ', n
print,'trigger completi presenti : ', tn
printf,25 ,'trigger completi presenti : ', tn
...
if (tn GT 0b) then begin		; CICLO DI CARICAMENTO :
					; ESEGUITO SE CI SONO FILE I DEI TRIGGER

					; CARICA IL NUMERO DEL TRIGGER
	...
	tstore = tp + 1
 	printf,25 ,'analizzato trigger n. ', (tp + 1)
...
oldtp = tp 
	...
	endelse					; OSSERVAZIONE
				...
				; SALVA IL NUMERO DEL TRIGGER
	
	close, 17 
	openw, 17, 'AcTrig.txt'
	print, 'prossimo trigger da caricare : n. ', tp+1 	
	printf, 17, tp
	close, 17 
...
;printf, 25
;printf, 25, 'il trigger e'' valido'
;printf, 25 

printf, 25
printf, 25, 'condizione =       ', condition
printf, 25
printf, 25, 'punteggio = ', punteggio
printf, 25
printf, 25, '------------------------------------------------------------------------------------'
printf, 25
close, 25 
...
plot,[1.0,0,0,0]##x,[1.0,0,0,0]##y,XTITLE='Time (s)',YTITLE='# counts',TITLE=orbit+' osservazione '+tempobs+' (Trigger n.'+STRCOMPRESS(STRING(tstore))+')',SUBTITLE='Shield n.1'
...
print, 'orbit = '+orbit
if ((tn EQ 1)OR((oldtp+1) EQ tn)) then begin
  ;for r = 0, 3 do spawn, 'rm '+filedaaprire(r)
  ;print, 'rm  ./'+orbit+'/*.tot.out'
  spawn, 'rm  ./'+orbit+'/*.tot.out'
endif 
...
close,4
print,' '
print,'File non valido. Elaborazione annullata.'
openu,1,'SDIresult.txt',APPEND=1
printf,1,' '
printf,1,'File non valido. Elaborazione annullata.'
close,1
goto,FINEFINALE 
...
REBIN:
for j=0,3 do begin
   if trig(j) EQ 1 then durminima=durata(j)
endfor
for j=0,3 do begin
   if (durata(j) LT durminima) AND (trig(j) EQ 1) then durminima=durata(j)
endfor
if durminima LT 0.2 then d=1 else d=fix(durminima*FattoreDK)
REBIN2:openu,1,'SDIresult.txt',APPEND=1
printf,1,' '
printf,1,'Fattore di rebin utilizzato: '+STRCOMPRESS(STRING(d))
close,1

k1(0)=LunghezzaFile/d	;**** correzione 2001 ****
...
openu,1,'SDIresult.txt',APPEND=1
printf,1,'Picco numero: '+STRCOMPRESS(STRING(numpicco))
close,1
...
openu,1,'SDIresult.txt',APPEND=1
printf,1,' '
printf,1,'Nessuna condizione di trigger'
close,1
goto,FINEFINALE
...
openu,1,'SDIresult.txt',APPEND=1
printf,1,' '
printf,1,'Il burst non termina mai'
close,1
openu,1,'risultati_finali.log',APPEND=1
printf,1,' '
printf,1,'--------------------------------------------------------------------'
printf,1,'Orbita: '+STRCOMPRESS(orbit(ilaria))
printf,1,'Trigger numero: '+STRCOMPRESS(STRING(franca+1))
printf,1,'Picco numero: '+STRCOMPRESS(STRING(numpicco))
printf,1,'Durata: The Neverending Burst'
printf,1,'Inizio:',tin
close,1
goto,FINEFINALE
...
binout3=[(LunghezzaFile-1.0-Step2Massimo),(LunghezzaFile-1.0-Step2Massimo),(LunghezzaFile-1.0-Step2Massimo),(LunghezzaFile-1.0-Step2Massimo)]
...
openu,1,'SDIresult.txt',APPEND=1
printf,1,' '
printf,1,'Trigger scattato:'
printf,1,'   Schermo   ','Tinizio      ','Sigma'
for j=0,3 do begin
   if trig(j) EQ 1 then printf,1,j+1,tin(j),b(j)
endfor
printf,1,' '
printf,1,'Il picco termina nei seguenti schermi nel seguente istante'
for j=0,3 do begin
   if trig(j) EQ 1 then printf,1,j+1,tfine(j),'1'
endfor
close,1
for j=0,3 do begin
   if trig(j) EQ 1 then begin
...
!X.RANGE=[x(binin3(j1)-64,j1),x(MIN([binout(j2)+64,LunghezzaFile-1]),j2)]
...
if t90end2(j) EQ 0.0 then begin
   t90end2(j)=x(binout(j2)+StepMinimo,j)
   fester(j)=1
endif
endfor
...
openu,1,'SDIresult.txt',APPEND=1
printf,1,' '
printf,1,'Durata evento:'
printf,1,'Minima:',durataeventoT90min
printf,1,'Media:',durataeventoT90media
if fester(jarea) EQ 0 then printf,1,'Massima:',durataeventoT90max
if fester(jarea) EQ 1 then printf,1,'Massima:',durataeventoT90max,'   (Attenzione: utilizzato T100)'
close,1
...
for j=0,3 do begin
   if dur(j) LE 0.0 then trig(j)=0
			...
			; **** salva le durate e i flag di segnale presente ****

if (numpicco EQ 1b)then begin
  save, filename = 'Dur_SDIres.dat', dur, trig
endif
...
openu,1,'SDIresult.txt',APPEND=1
printf,1,' '
printf,1,'Risultati della Cross-Correlazione:'
close,1
...
; ******   APPENA MINORE (correlP) ED APPENA MAGGIORE (correlD) ******
            ...
            if bandierina3 EQ 1 then begin
               ...
               openu,1,'SDIresult.txt',APPEND=1
               printf,1,jennifer+1,juliette+1,correlatioM,lag(k99)
               close,1
...
openu,1,'SDIresult.txt',APPEND=1
printf,1,' '
printf,1,'Punteggi ai vari schermi:'
close,1

punteggio_minimo=50

p = [-0.01, -0.01, -0.01, -0.01, -0.01, -0.01]	; VETTORE DEI PUNTEGGI
...
for jennifer=3,0,-1 do begin
   for juliette=3,0,-1 do begin
	 ...
	 if (numpicco EQ 1b) then begin
	   if (((jennifer+1) EQ 4) AND ((juliette+1) EQ 3)) then p[0] = punteggio
	   if (((jennifer+1) EQ 4) AND ((juliette+1) EQ 2)) then p[1] = punteggio
	   if (((jennifer+1) EQ 4) AND ((juliette+1) EQ 1)) then p[2] = punteggio
	   if (((jennifer+1) EQ 3) AND ((juliette+1) EQ 2)) then p[3] = punteggio
	   if (((jennifer+1) EQ 3) AND ((juliette+1) EQ 1)) then p[4] = punteggio
	   if (((jennifer+1) EQ 2) AND ((juliette+1) EQ 1)) then p[5] = punteggio
	 endif

	 close, 1
	 openu,1,'SDIresult.txt',APPEND=1
	 printf,1,' '
	 printf,1 ,jennifer+1,juliette+1,punteggio
	 close, 1
...
print,'Segnale sui seguenti schermi:'
...
for j=0,3 do begin
   ...
   if (beatrice(j) EQ 0) AND (trig(j) EQ 1) then print,j+1,tin(j),t90end(j),dur(j),massimo(j),tmassimo(j)
endfor

openu,1,'SDIresult.txt',APPEND=1
printf,1,' '
printf,1,'Segnale sui seguenti schermi:'
printf,1,' '
printf,1,'   Schermo   ','inizio       ','fine         ','durata       ','massimo      ','tmassimo'
for j=0,3 do begin
   if dur(j) LE 0 then beatrice(j)=0
   if beatrice(j) EQ 1 then printf,1,j+1,tin(j),t90end(j),dur(j),massimo(j),tmassimo(j),'  *'
   if (beatrice(j) EQ 0) AND (trig(j) EQ 1) then printf,1,j+1,tin(j),t90end(j),dur(j),massimo(j),tmassimo(j)
endfor
close,1

if (sabrina EQ 0) then begin
   sabrina=1
   d2=fix(durataeventoT90media*FattoreDK)
   if d2 LT 1 then d2=1
   if d2 EQ d then goto,PAGANINI
   d=d2
   goto,REBIN2
PAGANINI:
endif
...
if beatrice(0)+beatrice(1)+beatrice(2)+beatrice(3) GT 0 then begin
   openu,1,'risultati_finali.log',APPEND=1
   printf,1,' '
   printf,1,'--------------------------------------------------------------------'
   printf,1,'Picco numero: '+STRCOMPRESS(STRING(numpicco))
   printf,1,'Durata: '+STRCOMPRESS(STRING(durataeventoT90media))+' +'+STRCOMPRESS(STRING(durataeventoT90max-durataeventoT90media))+' -'+STRCOMPRESS(STRING(durataeventoT90media-durataeventoT90min))
   printf,1,'Punteggio minimo: '+STRCOMPRESS(STRING(punteggio_minimo))
   close,1
endif
...
if beatrice(0)+beatrice(1)+beatrice(2)+beatrice(3) GT 0 then begin
   print, ' '
   print,'--------------------------------------------------------------------'
   print,'Picco numero: '+STRCOMPRESS(STRING(numpicco))
   print,'Durata: '+STRCOMPRESS(STRING(durataeventoT90media))+' +'+STRCOMPRESS(STRING(durataeventoT90max-durataeventoT90media))+' -'+STRCOMPRESS(STRING(durataeventoT90media-durataeventoT90min))
   print,'Punteggio minimo: '+STRCOMPRESS(STRING(punteggio_minimo))
endif
        
;p =  [ p3p4, p2p4, p1p4, p3p2, p1p3, p1p2 ] ; PUNTEGGI 

if (numpicco EQ 1b)then begin
  save, filename = 'Poi_SDIres.dat', p, trig
endif

numpicco=numpicco+1b
...
FINEFINALE:
    ...
    if (condition NE 'valido') then begin
	print
	print, 'il trigger non e'' valido'
	print 
	
	punteggio = '100'
	printf, 25		
	printf, 25, 'condizione = ','corrotto'
	printf, 25, 'punteggio  = ',punteggio
	printf, 25
	printf, 25
	printf, 25, '------------------------------------------------------------------------------------'
...
endif	; FINE DEL CICLO DI ANALISI
